...
def fact_check(req: FactCheckRequest):
    ...
    for claim in claims:
        ...
        print(f"[API DEBUG] Intent: {intent}")

        # Entity + science override
        if entity and intent == "SCIENTIFIC":
            intent = "ENTITY_FACT"

        # News fallback - if intent classifier fails, detect news claims
        if intent == "UNKNOWN" and is_news_claim(claim):
            intent = "NEWS"
            print(f"[DEBUG] API: News fallback detected - {claim}")
        ...
        print(f"[API DEBUG] Evidence found: {len(evidence)} articles")
        for i, ev in enumerate(evidence[:3], 1):
            print(f"[API DEBUG]   {i}. {ev['title'][:50]}... (Source: {ev['source']})")
