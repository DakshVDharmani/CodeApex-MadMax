...
def load_nli_data_fast(filepath, max_samples=10000):
    """Load better quality NLI data"""
    ...
    for line in tqdm(lines, desc="Processing data"):
        ...
        if len(parts) == 3:
            ...
            if label in label_counts:
                # Filter out poor quality examples
                if len(premise.strip()) > 10 and len(hypothesis.strip()) > 10:
    ...
    # Use available classes for better training
    # If neutral is missing, use contradiction and entailment
    if label_counts['neutral'] == 0:
        print("No neutral samples found, using binary classification (contradiction/entailment)")
        min_count = min(label_counts['contradiction'], label_counts['entailment'])
        
        contradiction_data = [(p, h, l) for p, h, l in data if l == 'contradiction'][:min_count]
        entailment_data = [(p, h, l) for p, h, l in data if l == 'entailment'][:min_count]
        
        balanced_data = contradiction_data + entailment_data
        label_map = {'contradiction': 0, 'entailment': 1}
        
        print(f"Balanced dataset: {len(balanced_data)} samples")
        print(f"Contradiction: {len(contradiction_data)}, Entailment: {len(entailment_data)}")
    else:
        # Use all three classes
        min_count = min(label_counts['contradiction'], label_counts['neutral'], label_counts['entailment'])
        
        contradiction_data = [(p, h, l) for p, h, l in data if l == 'contradiction'][:min_count]
        neutral_data = [(p, h, l) for p, h, l in data if l == 'neutral'][:min_count]
        entailment_data = [(p, h, l) for p, h, l in data if l == 'entailment'][:min_count]
        
        balanced_data = contradiction_data + neutral_data + entailment_data
        label_map = {'contradiction': 0, 'neutral': 1, 'entailment': 2}
        
        print(f"Balanced dataset: {len(balanced_data)} samples")
        print(f"Contradiction: {len(contradiction_data)}, Neutral: {len(neutral_data)}, Entailment: {len(entailment_data)}")
...
def train_nli_fast():
    ...
    # Determine number of classes for model
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        config['model_name'], 
        num_labels=num_labels  # Dynamic number of classes
    )
