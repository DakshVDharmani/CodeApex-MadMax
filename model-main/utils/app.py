...
from models.live_sources.google_search import is_news_claim
...
def main():
    ...
    for idx, claim in enumerate(claims, 1):
        ...
        # -------------------------------
        # NEWS INTENT FALLBACK
        # -------------------------------
        # If intent classifier fails, check for news keywords
        if intent == "UNKNOWN":
            news_keywords = ["conflict", "talks", "blocked", "spikes", "economic", "political", "election", "protest", "summit", "negotiations", "diplomatic", "government", "officials", "announced", "military", "sanctions"]
            if any(kw in claim.lower() for kw in news_keywords):
                intent = "NEWS"
                print("[DEBUG] Intent fallback: NEWS (keyword detection)")
        
        # Final fallback: simple news detection
        if intent == "UNKNOWN" and is_news_claim(claim):
            intent = "NEWS"
            print("[DEBUG] Intent fallback: NEWS (simple detection)")
        ...
        # -------------------------------
        # DEBUG: Show evidence findings
        # -------------------------------
        if intent == "NEWS":
            print(f"[DEBUG] News evidence found: {len(evidence)} articles")
            for i, ev in enumerate(evidence[:3], 1):
                print(f"[DEBUG]   {i}. {ev['title'][:50]}... (Authority: {ev['authority']})")
